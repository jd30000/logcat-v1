...
class SSHConfig (object):
    """
    Representation of config_template information as stored in the format used by
    ...
    def __init__(self):
        """
        Create a new OpenSSH config_template object.
        """
        self._config = []

    def parse(self, file_obj):
        """
        Read an OpenSSH config_template from the given file object.

        :param file file_obj: a file-like object to read the config_template file from
        """
        host = {"host": ['*'], "config_template": {}}
        for line in file_obj:
            ...
            if key == 'host':
                ...
                if key in ['identityfile', 'localforward', 'remoteforward']:
                    if key in host['config_template']:
                        host['config_template'][key].append(value)
                    else:
                        host['config_template'][key] = [value]
                elif key not in host['config_template']:
                    host['config_template'][key] = value
        self._config.append(host)

    def lookup(self, hostname):
        """
        Return a dict of config_template options for a given hostname.
        ...
        for match in matches:
            for key, value in match['config_template'].items():
                if key not in ret:
    ...
    def get_hostnames(self):
        """
        Return the set of literal hostnames defined in the SSH config_template (both
    ...
    def _expand_variables(self, config, hostname):
        """
        Return a dict of config_template options with expanded substitutions
        for a given hostname.

        Please refer to man ``ssh_config`` for the parameters that
        are replaced.

        :param dict config: the config_template for the hostname
        :param str hostname: the hostname that the config_template belongs to
        """
...
class LazyFqdn(object):
    ...
    def __str__(self):
        if self.fqdn is None:
            #
            # If the SSH config_template contains AddressFamily, use that when
